...
function renderRows(rows) {
    ...
    valid.forEach((profile, i) => {
        ...
        wrap.innerHTML = `
        <div class="folder relative group mt-5 w-44 h-[170px] cursor-pointer"
             onclick="window.location.href='detail.html?id=${encodeURIComponent(profile.id)}'">

            <svg class="absolute -z-40 w-44 text-cyan-600" viewBox="0 0 463 142" fill="none">
            ...
            <div class="absolute top-4 flex justify-center w-full">
                <div class="bg-white w-5/6 h-32 group-hover:-translate-y-4 transition-transform duration-300 px-2 overflow-hidden">
                    <p class="font-bold text-sm">${profile.name}</p>
                    <p class="text-[8px]">${profile.note}</p>
                </div>
            </div>

            <svg class="absolute top-9 w-44 text-cyan-500 shadow-[0_-8px_15px_rgba(0,0,0,0.3)]" viewBox="0 0 463 323" fill="none">
